...
\setbeamertemplate{headline}{
  ...
  \begin{columns}[c]
    ...
    \begin{column}{.15\paperwidth}
      \centering
      \includegraphics[height=12cm,keepaspectratio]{logo_transparent.png}
    \end{column}
  \end{columns}
  \vskip2cm
}
...
\begin{block}{The Deepfake Threat \& Solution}
\begin{minipage}[t][32cm][c]{\linewidth}
    \vspace{0.5cm}
    \begin{itemize}
        \item[\textcolor{neoncyan}{\textbf{>}}] \Large \textbf{MFS e-KYC Bypass:} Fraudsters exploit synthetic media to bypass liveness checks in bKash, Nagad, and other digital wallets.
        \vspace{0.5cm}
        \item[\textcolor{neoncyan}{\textbf{>}}] \Large \textbf{Official Impersonation:} AI-generated videos impersonate officials and executives for targeted financial scams.
        \vspace{0.5cm}
        \item[\textcolor{neoncyan}{\textbf{>}}] \Large \textbf{Political Misinformation:} Deepfakes manipulate public discourse and erode trust during critical national events.
    \end{itemize}
    \vspace{1.5cm}
    \begin{center}
...
\begin{minipage}[t][31cm][c]{\linewidth}
    \begin{center}
        \includegraphics[height=28cm,keepaspectratio]{sleek_diagram.jpg}
    \end{center}
\end{minipage}
\end{block}
...
\begin{minipage}[t][32cm][c]{\linewidth}
    ...
    \resizebox{0.95\linewidth}{!}{
        \begin{tikzpicture}[node distance=3cm and 2cm]
            ...
            \node (fake) [io, right=of dec, draw=techred, text=techred, xshift=1.5cm] {\huge \textbf{SYNTHETIC}};
            \node (real) [io, left=of dec, draw=neongreen, text=neongreen, xshift=-1.5cm] {\huge \textbf{AUTHENTIC}};
            
            \draw [arrow] (in) -- (sample);
            \draw [arrow] (sample) -- (nn);
            \draw [arrow] (nn) -- (agg);
            \draw [arrow] (agg) -- (dec);
            \draw [arrow, techred] (dec) -- node[anchor=south, yshift=0.3cm, text=titletext] {\LARGE \textbf{Yes}} (fake);
            \draw [arrow, neongreen] (dec) -- node[anchor=south, yshift=0.3cm, text=titletext] {\LARGE \textbf{No}} (real);
        \end{tikzpicture}
    }
    \end{center}
\end{minipage}
...
\begin{minipage}[t][31cm][c]{\linewidth}
    \begin{center}
        \includegraphics[height=28cm,keepaspectratio]{heatmap.jpg}
    \end{center}
\end{minipage}
\end{block}
...
\begin{minipage}[t][32cm][c]{\linewidth}
    \begin{center}
        \includegraphics[height=22cm,keepaspectratio]{sleek_concept.jpg}
    \end{center}
    \vspace{1.5cm}
...
\begin{minipage}[t][31cm][c]{\linewidth}
    ...
    \resizebox{0.9\linewidth}{!}{
        \begin{tikzpicture}[node distance=3cm and 0cm]
            \node (trust) [rectangle, draw=neoncyan, fill=bgblock, text=white, very thick, minimum width=15cm, minimum height=4cm, align=center, drop shadow] {\Huge \textbf{Restoring Digital Trust} \\ \Large Empowering journalists to verify media authenticity.};
            
            \node (future) [rectangle, draw=neonmagenta, fill=bgblock, text=white, very thick, minimum width=15cm, minimum height=4cm, align=center, drop shadow, below=2cm of trust] {\Huge \textbf{The Road Ahead} \\ \Large Expanding to Multimodal Audio/Visual Detection.};
